#jurisdrtiations, country, region , workflows, name, status, global_enabled, id
import json
import csv
...
class DataNavigator:
    ...
    def write_into_csv(self):
        ...
        with open('all_templates_organized_pdf1.csv', "w", newline='') as _file:
            _data = self.get_all_info()
    ...
    def find_by_regions(self, region):
        _data = self.get_all_info()
        requested_templates = []
        ...
        for x in _data:
            juris = x['juris']
            for unit in juris:
                if unit['region'].lower() == region.lower():
...
organizer.find_by_regions("quebec")
organizer.write_into_csv()
